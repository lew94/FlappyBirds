...
class Pillars:
    ...
    def update(self):
        self.spawn_count += 1
        if self.spawn_count > Constants.pillar_spawn_rate:
            self.spawn()
            self.spawn_count = 0
...
class UpPillar(Pillar):
    def __init__(self, offset=0):
        ...
        self.image = pygame.image.load('assets/rockGrass.png').convert_alpha()
...
class DownPillar(Pillar):
    def __init__(self, offset=0):
        super().__init__()
        self.y_pos = offset
        self.image = pygame.image.load('assets/rockGrassDown.png').convert_alpha()
        self.mask = pygame.mask.from_surface(self.image)
        self.rect = self.image.get_rect()
        self.rect.center = (self.x_pos, self.y_pos)
